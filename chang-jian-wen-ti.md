...
**为什么Current需要使用区块链来实现它的目标？**

> **Why does Current need to use the blockchain to accomplish its goals?**

Current将利用区块链的优势跨越两个阶段，首先，让Current用户能够记录用户对平台的贡献，并奖励他们的时间、注意力和消费。这将不断演变为使用区块链来帮助去中心化和透明地分发内容、支付、商品和服务。
...
**为什么Current选择以太坊而不是其他区块链？**

> **Why did Current choose Ethereum over another blockchain?**

之所以选择Ethereum网络，是因为它是为应用程序集成而构建的最健壮的区块链平台之一，同时它背后有着最大的开发社区。

> The Ethereum network was chosen because it is one of the most robust blockchain platform built for application integration along with the largest development community behind it.

Ethereum基金会致力于不断开发和改进底层技术，以改善规模问题，这使Ethereum成为最引人注目的区块链平台之一。我们的团队一直在密切关注Raiden的进展，以及我们如何利用状态通道来解决我们在Current内面临的可伸缩性问题。
...
**当前的平台是否仅仅依赖于第三方API集成？**

> **Is the Current platform reliant solely on third party API integrations?**

目前，Current平台在2017年3月底推出了一版本，其中包括8个公共和私人集成，其中很大一部分是通过直接合作实现的。我们所有的集成合作伙伴都从中受益，因为他们保留了用户在Current内播放产生的所有交互、数据和收入数据，例如，Spotify上的一个视频仍然可以在Spotify上播放，而且他们可以在Spotify上获得报酬。
...
此外，由于Current应用的平均会话持续时间平均为18分钟，并且持续增加，这些集成可以期望用户在各自的网络上花费更长的时间，并为用户提供更好的体验。
...
我们的任务不是要蚕食我们的合作伙伴网络，事实上，我们希望我们的用户在这些网络中使用代币来获得高级订阅。这进一步使这些网络和整个行业受益，使免费用户成为付费用户。值得注意的是，我们的目标是随着时间的推移继续增加内容集成，进一步降低任何集成从Cuurent平台上移除可能带来的影响。
...
在平台的最后阶段，Current的平台将会达到用户的临界数量，它需要充分利用我们的去中心化媒体集成，与最受欢迎的第三方网络共存。

> During final phase of the platform, the Current platform will have reached the critical mass of users it needs to make full use of our decentralized media integration that would co-exist alongside the most popular third party networks.

**Current协议如何描述到平台的新集成？**

> **How does Current’s protocol account for new integrations into the platform?**

当添加新的集成时，特别是新媒体类型（例如电影），可用的相关信息池就会增长，从而增加我们的建议的广度。我们可以对更多内容进行分类和显示，从而使用户的参与度持续时间更长。这将反过来影响与每个用户和整个网络相关的贡献系数。
...
通过代币购买或通过媒体消费而获得的代币券，将能够在参与的交易所公开交易他们的CRNC。
...
**CRNC的总供给是多少？Current可能会发行更多吗？**

> **What is the total supply of CRNC? Will Current ever issue more?**

CRNC的总供应量将限制在10亿个代币，我们将永远不会在代币销售后再增发。在代币销售期间，所有的代币中有35%将用于购买，其余的将通过消费媒体的用户贡献来获得。用户能够获得这些代币的速度取决于代币动力学和协议部分中列出的各种因素。
...
CRNC代币不会将用户的钱包留在生态系统中，直到他们被发送到支持的二级交易所的钱包里，或者被放在一个支持的或专用的离线钱包里。

> The CRNC tokens won’t leave a user's wallet on the ecosystem until they’ve been sent to a wallet on a participating secondary exchange, or are placed in a participating or proprietary offline wallet.

在未来，第二阶段之后，代币将可以直接在其他平台上使用，用于高级服务、内容和其他商品。也就是说，一旦Current的广告平台上线，这些外部服务就可以利用他们积累的CRNC来购买Curent平台上的展示位广告。
...
**在Current平台上如何兑换CRNC代币？**

> **How are CRNC tokens redeemed in the platform?**

用户可以用CRNC订阅我们媒体合作伙伴（如Spotify）的高级服务。用户可以选择他们想要用CRNC兑换的服务，我们将提供一个升级他们在该网络上的服务的密钥。
...
**你如何消除用户玩游戏或滥用系统来持续赚取代币的可能性？**

> **How do you remove any chances of users gaming or abusing the system to continually earn tokens?**

为了消除“坏参与者”的潜在欺诈行为，在代币动力学和协议部分的第10页中列出的8个变量组成了我们的贡献系数，设计了一种自动检测和调整参与者赚取代币的速率的方法。如果一个“坏行为者”被自动检测到，他们的贡献系数将变为零，即使有效地参与也将得到零个CRNC代币。
...
在公开销售期间，将有350 000万CRNC代币或共代币供应量（1,000,000）的35%可用。
